...
namespace CorrelationPropagationDemos.WebApiDemo.Controllers
{
        ...
        public async Task<ActionResult<IEnumerable<KeyValuePair<string, string>>>> Get()
        {
            ...
            string incomingCv = 
                Program.UseActivityToPropagateCv ?
                    ...
                    HttpContext.GetCorrelationVector()?.Value;
            ...
            using ( HttpClient client = new HttpClient() )
            {
                ...
                if ( Program.UseActivityToPropagateCv )
                {
                    response = await client.SendAsync( request );
                }
                else
                {
                    response = await client.SendAsync( HttpContext.GetCorrelationVector(), request );
                }
            ...
            return
                new KeyValuePair<string, string>[]
                {
                    ...
                    new KeyValuePair<string, string>("On the Activity/HttpContext populated before controller code - Activity Id", incomingActivityId),
                    new KeyValuePair<string, string>("On the Activity/HttpContext populated before controller code - Correlation Vector", incomingCv),
